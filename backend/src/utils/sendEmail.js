...
export const sendEmail = async ({ email, subject, mailGenContent }) => {
  const mailGenerator = new Mailgen({
    ...
    product: {
      name: "AlgoCraft",
      link: "https://algocraft.tech",
      copyright: `Copyright © ${new Date().getFullYear()} AlgoCraft. All rights reserved.`,
    },
  });
...
export const emailVerificationMailGenContent = (fullname, verificationUrl) => ({
  body: {
    name: fullname,
    intro: "Welcome to AlgoCraft! We're very excited to have you on board.",
    action: {
...
export const forgotPasswordMailGenContent = (fullname, forgotPasswordUrl) => ({
  body: {
    name: fullname,
    intro: "You recently requested to reset your password for your AlgoCraft account.",
    action: {
